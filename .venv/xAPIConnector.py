...
class JsonSocket(object):
    def __init__(self, address, port, encrypt=False):
        ...
        if self._ssl:
            context = ssl.create_default_context()  # Crea un contexto SSL con configuraciones seguras predeterminadas.
            self.socket = context.wrap_socket(self.socket, server_hostname=address)  # Envuelve el socket existente en un socket SSL.
